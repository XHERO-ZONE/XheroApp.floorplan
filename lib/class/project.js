  ...
  _createClass(Project, null, [{
    ...
    value: function remove(state) {
      ...
      var _state$getIn = state.getIn(["scene", "layers", selectedLayer, "selected"]),
          ...
          selectedAreas = _state$getIn.areas;
      ...
      if (selectedAreas.toJS().length > 0) {
        var area = state.toJS().scene.layers[selectedLayer].areas[selectedAreas.toJS()[0]];

        var areaID = area.id;
        var lineVertex = Object.values(state.toJS().scene.layers[selectedLayer].lines).map(function (items) {
          return [{ id: items.id, arr: items.vertices }];
        });
        console.log(area.vertices);
        console.log(lineVertex);
        var matchingIds = [];

        lineVertex.forEach(function (element) {
          element.forEach(function (_ref2) {
            var id = _ref2.id,
                arr = _ref2.arr;

            var isMatch = arr.some(function (item) {
              return area.vertices.includes(item);
            });
            if (isMatch) {
              matchingIds.push(id);
              state = _export2.Area.remove(state, selectedLayer, areaID).updatedState;
              state = _export2.Line.remove(state, selectedLayer, id).updatedState;
            }
          });
        });
      }
